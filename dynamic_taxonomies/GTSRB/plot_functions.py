...
def evaluate_siamese(train_embeds,train_labels,calibration_embeds,calibration_labels,test_embeds,test_labels,config):
    ...
    confusion_matrix(calibration_labels,calibration_predictions,"kNN Confusion Matrix",(30,30),config.confusion_matrix_plot_path['calibration'])

    tsne = TSNE()
    train_tsne_embeds = tsne.fit_transform(train_embeds[:5000])
    scatter(train_tsne_embeds, train_labels[:5000], filename=config.scatter_plot_path['training'], subtitle="Samples from Triplet Training Data")
    calibration_tsne_embeds = tsne.fit_transform(calibration_embeds)
    scatter(calibration_tsne_embeds, calibration_labels, filename=config.scatter_plot_path['calibration'], subtitle="Samples from Triplet Calibration Data")
