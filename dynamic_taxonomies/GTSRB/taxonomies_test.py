...
def extract_results(CA,CLAP,CUAP,CE,LEP,UEP,type,taxon):
    ...
    accuracy_data={'sample': np.arange(len(CA))[downsample_idxs], 'CA':CA[downsample_idxs], 'CLAP':CLAP[downsample_idxs], 'CUAP':CUAP[downsample_idxs]}
    df=pd.DataFrame(accuracy_data,columns=['sample', 'CA', 'CLAP', 'CUAP'])
    path='Plots/experiment_{}/{}/{}_cumulative_accuracy_plot_data.csv'.format(C.experiment,type,taxon)
    df.to_csv(path, index=False, header=True)

    

def compute_single_pvalues(test_embeds):
    p_values=np.empty((len(test_embeds),C.num_classes))
    with open(C.calibration_nc_scores['nearest_centroid'], "rb") as f:
        calibration_nc,centroids=pickle.load(f)
    centroid_distances=np.zeros(C.num_classes)
...
def knn_v1(test_embeds,y_test,C):
    ...
    for type in ['static','dynamic_1','dynamic_2']:
        ...
        p_values=compute_single_pvalues(test_embeds)
