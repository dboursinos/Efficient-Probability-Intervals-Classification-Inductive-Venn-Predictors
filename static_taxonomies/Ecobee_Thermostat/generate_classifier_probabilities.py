...
calibration_probs=classifier_model.predict(data['x_validation'])
test_probs=classifier_model.predict(data['x_test'])

with open(C.classifier_probs_paths['train'], "wb") as f:
    pickle.dump(train_probs, f)

with open(C.classifier_probs_paths['validation'], "wb") as f:
    pickle.dump(calibration_probs, f)

with open(C.classifier_probs_paths['test'], "wb") as f:
