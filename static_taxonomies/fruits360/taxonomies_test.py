...
def nn2_nc2(x_test,y_test,classifier_model,siamese_model,C):
    with open(C.nn2_nc2_calibration_results_path, "rb") as f:
        centroids,category_distributions = pickle.load(f)

    num_nc2_categories=2*C.num_classes
    num_nn2_categories=2*C.num_classes
    
    test_embeds=siamese_model.predict(x_test)

    # place test data in categories
    test_probabilities=classifier_model.predict(x_test)
    test_predictions=np.argmax(test_probabilities,axis=1)
    test_prediction_probs=np.max(test_probabilities,axis=1)
    ...
    for i,test_embedding in enumerate(test_embeds):
        for j in range(C.num_classes):
            temp_distances[j]=np.linalg.norm(test_embedding-centroids[j])
        nn2_c=test_predictions[i]*2+(test_prediction_probs[i]>0.75)
        nc2_c=2*np.argmin(temp_distances)+int(np.min(temp_distances)>0.08)
        category=nc2_c*num_nn2_categories+nn2_c
    ...
    ax.plot(CUAP, 'k-', label = "CUAP")
    fig.legend()
    fig.savefig(C.nn2_nc2_ivp_results_plot_path)
    ...
    ax.plot(UEP, 'k-', label = "UEP")
    fig.legend()
    fig.savefig(C.nn2_nc2_ivp_results_error_plot_path)

    ce=evaluation_metrics.CE(y_test,test_venn_mean_all)
    bs=evaluation_metrics.BS(y_test,test_venn_mean_all)
    ece,mce=evaluation_metrics.ECE_MCE(y_test,test_venn_mean_all,10)

    with open(C.nn2_nc2_evaluation_metrics_path, 'w', newline='') as csvfile:
        fieldnames = ['Accuracy', 'CE', 'BS', 'ECE', 'MCE']
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)

        writer.writeheader()
        writer.writerow({'Accuracy':CA[-1], 'CE':ce, 'BS':bs, 'ECE':ece, 'MCE':mce})